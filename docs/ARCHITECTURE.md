...
| `mastery._generated_probe` | Authors a new check question (JSON) when the bank is exhausted | Structured output | Must pass a blind self-solve audit (`_self_check`) before a student ever sees it |
...
- **Hard caps guarantee termination.** 4 probe attempts, 12 model calls, and strategy-ladder exhaustion each independently force a teacher hand-off. The loop cannot run forever, regardless of model behavior.
...
- The 1B model produced a generated question with a wrong answer key. Response: bank-first probe selection plus the blind self-solve audit on anything generated.
