...
\begin{frame}
  \begin{center}
    ...
    {\rmfamily\fontsize{34}{38}\selectfont\color{ivory} Gemma Monsters\par}
    \vspace{0.6em}
...
\begin{frame}{The game world: the Citadel}
  {\small A gothic castle nexus in three.js --- drag to orbit. Five monsters idle on floating platforms, each guarding one curriculum strand and a sealed \textcolor{gold}{golden gate}. Someone is locked in the keep; defeat all five and the finale opens the gate.}\par
  \vspace{0.5em}
  \renewcommand{\arraystretch}{1.22}
  {\footnotesize
  \begin{tabular}{@{}p{2.1cm}p{3.6cm}p{7.2cm}@{}}
    \toprule
...
\begin{frame}{The player journey}
  ...
  \begin{tikzpicture}[
    ...
    \node[box, above right=0.3cm and 0.42cm of train.east, anchor=west] (win) {\color{okg}Mastery\\ \tiny Gemma forges a relic;\\ \tiny the strand falls};
    \node[box, below right=0.3cm and 0.42cm of train.east, anchor=west] (esc) {Escalation\\ \tiny the Collector is summoned\\ \tiny + teacher report};
    \draw[arr] (enc) -- (battle);
...
\begin{frame}{An agent, not a chatbot}
  \begin{columns}[T]
    ...
    \end{column}
    \begin{column}{0.42\textwidth}
      \textbf{The reasoning gate:} a right answer with shaky typed reasoning \textbf{does not count toward mastery} --- Gemma grades the explanation (closed labels), not just the bubble.\par
      \vspace{0.6em}
      \textbf{Explainable everywhere:} every agent decision is shown with its why --- \mutedtext{\small ``Trying a visual walkthrough --- you said you can't see why the denominators matter.''}\par
      \vspace{0.6em}
      \textbf{It cannot run away:} 4-attempt cap, 12-model-call budget, strategy exhaustion $\rightarrow$ clean hand-off.\par
      \vspace{0.3em}
...
\begin{frame}{Where Gemma does the thinking \hfill {\normalsize\color{muted}rubric: Gemma Integration, 30\%}}
  ...
  \begin{tabular}{@{}p{4.0cm}p{9.3cm}@{}}
    ...
    Author fresh probe questions & bank-first; must pass a blind self-solve audit or is discarded \\
    ...
    Write the teacher report & narrative only; every number comes from the session log \\
  ...
  \begin{itemize}\setlength\itemsep{0.15em}
    ...
    \item {\small One auditable door (\texttt{gemma\_client.py}); model size is a dial: \texttt{gemma3:12b} primary, \texttt{gemma3:1b} light.}
  \end{itemize}
\end{frame}
...
\begin{frame}{The Collector, his lieutenants, and the Gemma coach}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{\textcolor{gold}{THE COLLECTOR}} --- {\small a giant skull boss, summoned when the mastery loop escalates. A client-side mental-math speed trial: 3 lives, he attacks on every miss. Alongside him, Gemma writes the teacher report --- the agent knows its limits.}\par
      \vspace{0.6em}
      ...
      \end{itemize}
      {\small 90-second war-clock skirmishes with streak escalation; every miss and its latency is logged.}
    \end{column}
...
\begin{frame}{We caught our model failing --- and engineered around it}
  ...
  \begin{tabular}{@{}p{5.6cm}p{6.6cm}@{}}
    ...
    \textbf{Invented wrong arithmetic} in an explanation ($\tfrac{2}{3}+\tfrac{1}{4}=\tfrac{17}{12}$) & grounding rule: explain \emph{why}, never recompute --- all numbers come from the verified solution \\
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}\setlength\itemsep{0.45em}
        \item Gemma runs locally via \textbf{Ollama}: \texttt{gemma3:12b} primary, \texttt{gemma3:1b} light --- one environment variable. The demo works in airplane mode.
        \item The 3D world is local too: \textbf{three.js and the CC0 Quaternius monsters are vendored} --- no CDN, no asset fetches.
        \item The soundscape is \textbf{synthesized procedurally in WebAudio} --- zero audio files.
        \item Tutoring data is a protected student record --- \textbf{what a student struggles with stays on their machine.}
      \end{itemize}
      \vspace{0.15em}
      {\scriptsize\color{muted}Companion Kaggle notebook runs Gemma 4 12b-it on a cloud GPU --- the app itself never leaves the laptop.}
    \end{column}
    \begin{column}{0.42\textwidth}
      \begin{tikzpicture}
        \node[draw=line, fill=card, rounded corners=3pt, inner sep=10pt, text width=4.6cm, align=center] {
          ...
          {\scriptsize\color{muted}battles · agent loop · coach · reports}\\[3pt]
          {\color{clay}$\updownarrow$}\\[2pt]
          {\small Ollama + Gemma}\\
          {\scriptsize\color{muted}12B primary · 1B one flag away}\\[6pt]
...
\begin{frame}{What's next \hfill {\normalsize\color{muted}same architecture, deeper world}}
  \begin{itemize}\setlength\itemsep{0.6em}
    \item \textbf{Local tool use} --- Gemma emits constrained JSON tool calls; a local SymPy verifier proves every check deterministically. \mutedtext{\small (tool use for a model that doesn't natively support it)}
    ...
    \item \textbf{Classroom mode} --- teacher reports aggregate across a class, still local-first: the school's data stays in the school.
  \end{itemize}
  \vspace{0.7em}
