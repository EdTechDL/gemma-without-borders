  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}\setlength\itemsep{0.7em}
        \item Every Ontario Grade 9 student writes the EQAO math assessment. Many arrive with \textbf{specific, well-documented misconceptions} --- not general weakness.
        ...
        \item Teachers can't do this per-student, per-misconception. Generic review apps don't even try.
      \end{itemize}
    \end{column}
    \begin{column}{0.41\textwidth}
      \begin{tikzpicture}[scale=0.9]
        \node[draw=line, fill=card, rounded corners=2pt, inner sep=8pt, text width=5.2cm, align=left] at (0,0) {
          ...
          {\scriptsize\color{muted} Every wrong option is the exact result of a \emph{known} misconception --- so the wrong answer itself is the diagnosis.}
        };
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\begin{frame}{What we built}
  ...
  \begin{tikzpicture}[
    ...
    \node[box, right=of quiz] (diag) {Diagnose\\ \scriptsize misconception per mistake};
  ...
  \begin{itemize}\setlength\itemsep{0.4em}
    \item Student takes a 5-question quiz. On submit, the agent reads \textbf{all} the mistakes, finds the pattern, and picks the \textbf{priority misconception}.
...
\begin{frame}{An agent, not a chatbot}
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      ...
      \vspace{0.3em}
      \mutedtext{\small ``Two consecutive fresh questions answered correctly, with reasoning that shows the misconception is gone.''}\par
      \vspace{0.8em}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\setlength\itemsep{0.45em}
        ...
        \item {\small 48-misconception taxonomy, each with the flawed thinking spelled out}
        \item {\small every wrong option tagged with the misconception that produces it}
...
\begin{frame}{The 90-second live demo}
  \begin{enumerate}\setlength\itemsep{0.55em}
    ...
    \item {\small \textbf{Results} --- the agent finds the pattern, names the misconception, builds the study guide.}
