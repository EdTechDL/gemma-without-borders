  ...
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
      ...
      \begin{tikzpicture}
        ...
        \node[codebox, minimum width=2.9cm] (probe) at (3.35,0)
...
\begin{frame}{Gemma judges. Code guarantees.}
  ...
  \begin{tikzpicture}
    \node[gembox, align=left, text width=6.6cm, minimum height=4.4cm, font=\small] (gem) at (0,0) {
      ...
      authors fresh probes (audited)\\[2pt]
...
\begin{frame}{Scored against the rubric}
  ...
  \begin{columns}[T]
    \begin{column}{0.24\textwidth}
      {\color{coral}\small\textbf{USE OF GEMMA}\ {\bfseries 30}}\par
      ...
      All guardrailed; fully local via Ollama --- \texttt{gemma3:12b}, with \texttt{1b} one flag away.}
    \end{column}
    \begin{column}{0.24\textwidth}
      {\color{gold}\small\textbf{INNOVATION}\ {\bfseries 30}}\par
      ...
      Mastery gated on the student's own typed words --- and the monster
      answers those words back.}
    \end{column}
    \begin{column}{0.24\textwidth}
      {\color{teal}\small\textbf{FUNCTIONALITY}\ {\bfseries 20}}\par
      \vspace{0.4em}{\color{teal}\hrule height 1.2pt}\vspace{0.5em}
      {\footnotesize
      Runs live, end to end, Wi-Fi off: Citadel, battles, agent loop,
      skirmishes, boss trial, finale.\\[3pt]
      Hard caps: no session can hang; a model hiccup never hurts the student.}
    \end{column}
    \begin{column}{0.24\textwidth}
      {\color{cream}\small\textbf{WRITEUP}\ {\bfseries 20}}\par
      ...
      Guardrail table per call site, plus a Kaggle notebook that
      reproduces the loop.}
    \end{column}
  \end{columns}
  \vspace{0.3em}
  \begin{center}
    \small\dimt{One sentence for the scorecard:}\ \goldb{an autonomous, explainable, fully on-device
